...
message = tk.Label(window, text="Face-Recognition Attendance System" ,fg="gold"  ,width=63  ,height=1,font=('times', 30, 'italic bold ')) 
...
lbl = tk.Label(window, text="Employee ID",width=20  ,height=1  ,fg="white"  ,bg="saddle brown" ,font=('times', 20, ' bold ') ) 
lbl.place(x=350, y=150)

txt = tk.Entry(window,width=30 ,bg="white" ,fg="black",font=('times', 20, ' bold '))
txt.place(x=700, y=150)

lbl2 = tk.Label(window, text="Employee Full Name",width=20  ,fg="white"  ,bg="saddle brown"    ,height=1 ,font=('times', 20, ' bold ')) 
...
def TakeImages(): 
    if (len(txt.get()) != 0) and (len(txt2.get()) != 0):
        ...
        if(is_number(Id) and final.isalpha()):
            ...
            if(Id.isalpha()):
                res = "Please enter numeric employee id"
                message.configure(text= res)
            elif(is_number(name)):
